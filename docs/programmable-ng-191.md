...
- `haiagaru-ng191-0.5.mpp` をローカルパッチとしてMorphe Managerへ読み込み、元APKへ `Haiagaru` を適用します。
...
191 devの通常NG編集画面で使う追加フックが、上書き済みのp1レジスタをViewとして渡す問題を修正しました。フラグメント自身のみを渡し、getView()からルートを取得します。通常のNG登録・保存処理は変更しません。DEX検査で引数と元処理の維持を確認しています。実機での操作確認は未実施です。
