...
- `haiagaru-ng191-0.2.mpp` をローカルパッチとしてMorphe Managerへ読み込み、元APKへ `Haiagaru` を適用します。
...
| createdAt | エッヂ・5ch.net・2ch.sc・bbspink.comのUnix秒キーからDateを生成。それ以外はnull |
...
- 複数ルールのOR判定、個別OFF、対象板の完全一致、全板、スレ／レス分離、エラーの独立性、設定保存形式、旧設定移行、未知の板でのnull値をJVM上でテスト。
